		...
		Ticket.findById(req.params.id)
			...
			.then((ticket) => {
				...
				if (!stringMembers.includes(req.user.id)) {
					return res
						.status(404)
						.json({ notAllowed: "You are not allowed to see this ticket" });
				}
...
// @route DELETE api/tickets/clear/
// @desc DELETE all done tickets
// @access Private
router.delete(
	"/clear",
	passport.authenticate("jwt", { session: false }),
	...
	(req, res) => {
		Ticket.findById(req.params.id).then((ticket) => {
			if (ticket.createdBy == req.user.id) {
				...
				res.status(404).json({
					notPermitted: "You are not permitted to delete this ticket",
				});
			}
		});
	}
